...
const API_CONFIG = window.STELLARADS_CONFIG?.api || {
...
const TokenService = {
  getToken() {
    return localStorage.getItem(window.STELLARADS_CONFIG?.auth?.tokenKey || 'stellarads_token');
  },
  
  setToken(token) {
    localStorage.setItem(window.STELLARADS_CONFIG?.auth?.tokenKey || 'stellarads_token', token);
  },
  
  removeToken() {
    localStorage.removeItem(window.STELLARADS_CONFIG?.auth?.tokenKey || 'stellarads_token');
  },
...
const UserService = {
  getCurrentUser() {
    const userJson = localStorage.getItem(window.STELLARADS_CONFIG?.auth?.userKey || 'stellarads_user');
    return userJson ? JSON.parse(userJson) : null;
  },
  
  setCurrentUser(user) {
    localStorage.setItem(window.STELLARADS_CONFIG?.auth?.userKey || 'stellarads_user', JSON.stringify(user));
  },
  
  removeCurrentUser() {
    localStorage.removeItem(window.STELLARADS_CONFIG?.auth?.userKey || 'stellarads_user');
  }
};
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const protectedRoutes = window.STELLARADS_CONFIG?.auth?.protectedRoutes || [
    '/post-ad.html',
    '/affiliate.html',
    '/account.html'
  ];
  ...
  if (!isAuthenticated && protectedRoutes.some(route => currentPath.endsWith(route))) {
